...
export default function ItemBox({ product, addToCart, recommended, loading }) {
    ...
    return (
        <Link
            href={loading ? "#" : `/store/products/${product.id}`}
            className={`relative rounded-[24px] bg-[#F5F5F5] hover:bg-[#FDEDD6] ease-out duration-300 flex flex-col justify-between p-6 ${recommended ? "w-full" : "w-full md:w-[calc(50%-1rem)] lg:w-[calc(33%-1rem)]"}`}
        >
            {loading ? (
                <div className="flex items-center justify-center w-full lg:h-[372px] h-72 p-6">
                    <Spinner size='medium' color='outline' />
                </div>
            ) : (
                <>
                    {product.discount && (
